...
function Bubble(option) {
    ...
    this.x = this.randomX = option.x || 0
    this.y = this.randomY = option.y || 0
}
Bubble.prototype.draw = function(ctx, randomMove) {
    var x, y
    if(randomMove) {
        this.randomX += ~~ (Math.random() * (Math.random() > 0.5 ? 1: -1) * 10)
        this.randomY += ~~ (Math.random() * (Math.random() > 0.5 ? 1: -1) * 10)
        ctx.beginPath()
        ctx.arc(this.randomX, this.randomY, this.radius, 0, 2 * Math.PI, false)
...
function canvasCircleInit(canvas) {
    ...
    function randomMove(ctx) {
        ...
        self.window.requestAnimationFrame(randomMove)
    }
